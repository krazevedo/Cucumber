module Wait
	...
	def wait_for_allow
		counter = 0
		teste = page.has_selector?("md-tab-item > span")
	 	while teste == false
	  		counter += 1
	  		visit path_to('expenses')
	   		teste = has_selector?("md-tab-item > span")
	   		raise "Home doesn't visible than 3 seconds." if counter >= 5	    
	  	end
	end
